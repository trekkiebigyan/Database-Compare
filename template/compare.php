...
<head>
    <meta charset="utf-8">
    <title>Database schema compare tool</title>
...
<body>
    ...
    <div class="compare-database-block">
        <h1>Database schema compare tool</h1>
        <table class="table">
            <tr class="panel">
                <td>
                    <?php
                ...
                <tr class="data">
                    <?php foreach (array('fArray', 'sArray') as $blockType) { ?>
                        <td class="type-<?php echo $_REQUEST['action']; ?>">
                            ...
                            <?php } ?>
                            <?php if ($data != null && isset($data[$blockType]) && $data[$blockType] != null && count($data[$blockType]) && in_array($_REQUEST['action'], array('tables', 'views'))) { ?><a target="_blank" onclick="Data.getTableData('index.php?action=rows&baseName=<?php echo $basesName[$blockType]; ?>&tableName=<?php echo $tableName; ?>'); return false;" href="#" class="sample-data">Sample data (<?php echo SAMPLE_DATA_LENGTH; ?> rows)</a><?php } ?>
                        </td>
                    <?php } ?>
                </tr>
            <?php } ?>
